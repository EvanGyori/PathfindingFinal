...
{	
		...
		throws FileNotFoundException
	{
		...
		while (layoutInput.hasNextLine()) {
			...
			for (int i = 0; i < lineInput.length(); i++) {
				PointType pointType = getPointType(lineInput.charAt(i));
				if (pointType == null) {
					throw new IllegalArgumentException("File has improper characters");
				} else if (pointType == pointType.START) {
					start = new Point(i, sizeY - 1);
				} else if (pointType == pointType.DESTINATION) {
					destination = new Point(i, sizeY - 1);
				}
			}
		}
	...
	*/
	...
	public static int indexToX(int positionIndex)
	{
		return positionIndex % getWidth();
	}
	
	public static int indexToY(int positionIndex)
	{
		return positionIndex / getWidth();
	}
	*/
	
	public PointType getPoint(int x, int y)
	{
